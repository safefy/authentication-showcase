...
class JobRequestCreateViewModel
@Inject
constructor(private val jobRequestRepository: JobRequestRepository,
            private val roomRepository: RoomRepository)
    : DisposableViewModel(), StatusViewModel<JobRequestCreateViewModel.Status, JobRequestCreateViewModel.ViewDataBundle> {
    ...
    @MainThread
    fun createJobRequestRequest(createJobRequestRequest: CreateJobRequestRequest) {
        mutableStatus.value = Status.Loading(ViewDataBundle())
        addDisposable(
                jobRequestRepository.createJobRequest(createJobRequestRequest)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe({ opt ->
                            when {
