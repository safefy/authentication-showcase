...
import th.co.knightfrank.data_java.data.mapper.UserRoomInfoResponseMapper
...
class ChangeUserRoomListViewModel
@Inject
constructor(private val userRepository: UserRepository)
    ...
    @MainThread
    fun changeUserRoomRequest(changeUserRoomRequest: ChangeUserRoomRequest) {
        mutableStatus.value = Status.Loading(ViewDataBundle())
        addDisposable(
                userRepository.changeUserRoom(changeUserRoomRequest)
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
                        .subscribe({ opt ->
                            when {
    ...
    sealed class Status(viewDataBundle: ViewDataBundle) : StatusViewModel.Status<ViewDataBundle>(viewDataBundle) {
        ...
        class LoadUserInfoSuccess(viewDataBundle: ViewDataBundle) : Status(viewDataBundle)
    }

    data class ViewDataBundle(val headerResponse: HeaderResponse? = null,
                              val recordCount: Int? = null,
                              val userRoomInfoList: MutableList<UserRoomInfoParcel>? = mutableListOf<UserRoomInfoParcel>())
}