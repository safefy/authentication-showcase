...
import th.co.knightfrank.data_java.data.mapper.MessageInfoResponseMapper
...
constructor(private val messageRepository: MessageRepository)
    : DisposableViewModel(), StatusViewModel<MessageListViewModel.Status, MessageListViewModel.ViewDataBundle> {

    private val messageInfoResponseMapper = MessageInfoResponseMapper()
    ...
    @MainThread
    fun getInboxMessageByUserIDRequest(getInboxMessageByUserIDRequest: GetInboxMessageByUserIDRequest) {
        mutableStatus.value = Status.Loading(ViewDataBundle())
        addDisposable(
                messageRepository.getInboxMessageByUserID(getInboxMessageByUserIDRequest)
                            ...
                            when {
                                opt._data!!._responseHeader!!._responseCode == "000" -> {
                                    ...
                                    mutableStatus.value = Status.Success(
                                            currentValue.viewDataBundle.copy(
                                                    ...
                                                    inboxMessageList = getList(opt._data!!._responseBody)
                                            )
                                    )
                                }
                                opt._data!!._responseHeader!!._responseCode == "001" ->
                                    mutableStatus.value = Status.SuccessWithNoData(
                                            currentValue.viewDataBundle.copy(
                                                    headerResponse = opt._data!!._responseHeader!!,
                                                    inboxMessageList = mutableListOf()
                                            )
                                    )
                                else ->
    ...
    private fun getList(responseList: MutableList<InboxMessageInfoResponse>?): MutableList<InboxMessageInfoParcel> {
        val inboxMessageList: MutableList<InboxMessageInfoParcel> = mutableListOf()

        responseList?.forEach {
            inboxMessageList.add(
                    messageInfoResponseMapper.transform(it)
            )
        }
    ...
    data class ViewDataBundle(val headerResponse: HeaderResponse? = null,
                              val inboxMessageList: MutableList<InboxMessageInfoParcel>? = mutableListOf<InboxMessageInfoParcel>())
}