...
constructor(private val messageRepository: MessageRepository)
    : DisposableViewModel(), StatusViewModel<MessageDetailViewModel.Status, MessageDetailViewModel.ViewDataBundle> {
    ...
    @MainThread
    fun getInboxMessageDetailRequest(getInboxMessageDetailRequest: GetInboxMessageDetailRequest) {
        mutableStatus.value = Status.Loading(ViewDataBundle())
        addDisposable(
                messageRepository.getInboxMessageDetail(
                                ...
                                opt._data!!._responseHeader!!._responseCode == "000" -> {
                                    mutableStatus.value = Status.Success(
                                            currentValue.viewDataBundle.copy(
                                                    ...
                                                    messageDetailData = messageInfoResponseMapper.transform(opt._data?._responseBody)
                                            )
                                    )
                                }
    ...
    data class ViewDataBundle(val headerResponse: HeaderResponse? = null,
                              val messageDetailData: InboxMessageInfoParcel? = null)
}